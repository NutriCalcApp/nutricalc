...
// DESIGN SYSTEM
const C = {
  bg:"#080C12", surf:"#0F1520", card:"#141C2B", cardHi:"#1A2540",
  bord:"#1E2A3D", bord2:"#2A3A52",
  acc:"#10F07A", aLo:"#10F07A18", aLo2:"#10F07A28",
  blu:"#4D9EFF", bLo:"#4D9EFF18",
  ora:"#FF8C42", oLo:"#FF8C4218",
  red:"#FF4757", rLo:"#FF475718",
  yel:"#FFD60A", yLo:"#FFD60A18",
  txt:"#ECF2FF", mid:"#7A8BA8", dim:"#141C2B",
  // Gradient helpers
  g1:"#10F07A", g2:"#4D9EFF",
};
const FONTS = `
...
@keyframes glow{0%,100%{box-shadow:0 0 0 0 #10F07A00}50%{box-shadow:0 0 24px 6px #10F07A55}}
@keyframes glowBar{0%,100%{box-shadow:0 0 0 0 #10F07A00}50%{box-shadow:0 0 10px 2px #10F07A66}}
...
const cS = { background:C.card, borderRadius:22, border:`1px solid ${C.bord}`, padding:"18px 20px", marginBottom:12, boxShadow:"0 2px 16px rgba(0,0,0,.25), 0 1px 0 rgba(255,255,255,.03) inset" };
const bP = { width:"100%", padding:"16px 24px", background:`linear-gradient(135deg, ${C.acc} 0%, #0EC060 100%)`, color:"#080C12", border:"none", borderRadius:18, fontWeight:900, fontSize:16, cursor:"pointer", fontFamily:ff, letterSpacing:-0.2, transition:"opacity .15s, transform .1s", boxShadow:`0 4px 20px ${C.acc}44` };
  ...
  return (
    <div style={{marginBottom:16}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
        <span style={{fontSize:12,color:C.mid,fontWeight:600,letterSpacing:.3}}>{label}</span>
        <span style={{fontSize:13,fontWeight:800,color:over?C.red:C.txt}}>{rnd(val)}g <span style={{color:C.mid,fontWeight:400,fontSize:11}}>/ {target}g</span></span>
      </div>
      <div style={{background:C.dim,borderRadius:8,height:8,overflow:"hidden"}}>
        <div style={{width:`${pct}%`,background:`linear-gradient(90deg,${barColor},${barColor}cc)`,borderRadius:8,height:8,transition:"width .6s cubic-bezier(.4,0,.2,1)",boxShadow:isGood?`0 0 8px ${barColor}88`:"none"}}/>
      </div>
    </div>
  );
}
...
function BottomNav({tab,setTab,lang,setSubScreen}) {
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:`${C.surf}EE`,borderTop:`1px solid ${C.bord}`,display:"flex",padding:"10px 8px 22px",zIndex:50,backdropFilter:"blur(12px)"}}>
      {tabs.map(([id,ic,lbl])=>(
        <button key={id} onClick={()=>{setTab(id);setSubScreen&&setSubScreen(null);}}
          style={{flex:1,background:"none",border:"none",color:tab===id?C.acc:C.mid,cursor:"pointer",padding:"4px 0",fontFamily:ff,position:"relative",transition:"color .2s"}}>
          {tab===id&&<div style={{position:"absolute",top:-10,left:"50%",transform:"translateX(-50%)",width:32,height:3,borderRadius:2,background:`linear-gradient(90deg,${C.acc},${C.blu})`,boxShadow:`0 0 8px ${C.acc}88`}}/>}
          <div style={{fontSize:22,transition:"transform .2s",transform:tab===id?"scale(1.15)":"scale(1)"}}>{ic}</div>
          <div style={{fontSize:10,fontWeight:tab===id?800:500,marginTop:2,letterSpacing:.2}}>{lbl}</div>
        </button>
      ))}
    </div>
  );
        ...
        return (
          <div key={name} onClick={()=>onMealClick(name)}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:cnt>0&&!isCustomized?10:0}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:50,height:50,background:`${["Colazione","breakfast"].some(k=>name.includes(k))?C.ora:["Pranzo","lunch"].some(k=>name.includes(k))?C.acc:["Cena","dinner"].some(k=>name.includes(k))?C.blu:"#C77DFF"}18`,borderRadius:15,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,border:`1.5px solid ${["Colazione","breakfast"].some(k=>name.includes(k))?C.ora:["Pranzo","lunch"].some(k=>name.includes(k))?C.acc:["Cena","dinner"].some(k=>name.includes(k))?C.blu:"#C77DFF"}28`,flexShrink:0}}>{icon}</div>
                <div>
              ...
              <div style={{textAlign:"right",display:"flex",alignItems:"center",gap:8}}>
                ...
                <div>
                  <div style={{fontWeight:900,color:cnt>0?C.acc:C.dim,fontSize:20,letterSpacing:-0.5}}>{cnt>0?rnd(tot2.cal):"—"}</div>
...
export default function App() {
  ...
  const generateMeal=name=>{
    ...
    _el.style.cssText='position:fixed;bottom:90px;left:50%;transform:translateX(-50%);background:#080C12;color:#10F07A;padding:10px 18px;border-radius:12px;font-weight:700;font-size:13px;border:1px solid #10F07A44;z-index:999;pointer-events:none';
  ...
  return (
    <div key={lang} style={{...ss,paddingBottom:70}}>
      ...
      {tab==="profile"&&<ProfileScreen profile={profile} setProfile={setProfile} targets={targets} user={user} weightLog={weightLog} newWeight={newWeight} setNewWeight={setNewWeight} onLog={manualLogWeight} onCalc={recalc} onLogout={handleLogout} onReset={handleReset} onManualMacros={manualMacros} onSaveExcluded={pr=>{setProfile(pr);saveProfile(pr);}} onSaveProfile={pr=>{setProfile(pr);saveProfile(pr);}} lang={lang} onChangeLang={changeLang} onShowProgress={()=>{setSelMeal(null);setTab("progress");}} onOpenPiano={()=>{setSelMeal(null);setSubScreen("piano");}} onOpenImport={()=>{setSelMeal(null);setSubScreen("import");}} onOpenDietLibrary={()=>setShowDietLibrary(true)}/>}
