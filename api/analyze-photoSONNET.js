...
export default async function handler(req, res) {
  ...
  const prompt = `Sei un nutrizionista digitale. Analizza questa foto di un pasto e identifica tutti gli alimenti visibili.
...
Usa emoji appropriate. Per liquidi usa "ml" come unit.`;
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(geminiRes.status).json({ error: 'Gemini error: ' + errText.slice(0, 300) });
    }
