...
export default function App() {
  ...
  const applyPresetDiet=(diet)=>{
    ...
    setMeals(protectedMeals); saveMeals(protectedMeals);
    const protectedPlan=newPlan.map((day,di)=>{if(di!==todayIdx)return day;const pd={...day};activeMealList.forEach(m=>{if((lockedMeals||{})[m.name])pd[m.name]=meals[m.name]||[];});return pd;});
    setWeeklyPlan(protectedPlan); LS.s("nc2-weeklyplan",protectedPlan);
  ...
  const applySavedPlan=(sp)=>{
    ...
    setMeals(protectedMeals); saveMeals(protectedMeals);
    const protectedPlan=sp.planData.map((day,di)=>{if(di!==todayIdx)return day;const pd={...day};mealList.forEach(m=>{if((lockedMeals||{})[m.name])pd[m.name]=meals[m.name]||[];});return pd;});
    setWeeklyPlan(protectedPlan); LS.s("nc2-weeklyplan",protectedPlan);
    setIsCustomized(false); setShowDietLibrary(false); setSubScreen("piano");
  };
  ...
  const handleGeneratePlan=(seed)=>{
    ...
    const protectedMeals=mealList.reduce((acc,x)=>{acc[x.name]=(lockedMeals||{})[x.name]?(meals[x.name]||[]):[];return acc;},{});
    setMeals(protectedMeals); saveMeals(protectedMeals);
    const protectedPlan=plan.map((day,di)=>{if(di!==todayIdx)return day;const pd={...day};mealList.forEach(m=>{if((lockedMeals||{})[m.name])pd[m.name]=meals[m.name]||[];});return pd;});
    setWeeklyPlan(protectedPlan);
    setPlanSeed(newSeed);
    LS.s("nc2-weeklyplan",protectedPlan);
  ...
  const handleGeneratePlanFromPantry=()=>{
    ...
    const protectedMeals2=mealList.reduce((acc,x)=>{acc[x.name]=(lockedMeals||{})[x.name]?(meals[x.name]||[]):[];return acc;},{});
    setMeals(protectedMeals2); saveMeals(protectedMeals2);
    const protectedPlan2=plan.map((day,di)=>{if(di!==todayIdx)return day;const pd={...day};mealList.forEach(m=>{if((lockedMeals||{})[m.name])pd[m.name]=meals[m.name]||[];});return pd;});
    setPlanSeed(newSeed);
    LS.s("nc2-planseed",newSeed);
    setWeeklyPlan(protectedPlan2);
    LS.s("nc2-weeklyplan",protectedPlan2);
    setIsCustomized(false);
  };
